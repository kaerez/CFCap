...
export default {
  async fetch(request, env, ctx) {
      ...
      storage: {
        challenges: {
          store: async (token, challengeData) => {
            // SQLite INSERT OR REPLACE is equivalent to Postgres ON CONFLICT DO UPDATE
    ...
    // Example verification endpoint (optional, for your backend logic)
    if (request.method === "POST" && url.pathname === "/api/validate") {
      try {
        const { token } = await request.json();
        // You can pass { keepToken: true } if you want to reuse it, default is false (consume it)
    ...
    // Serve static assets (via Assets binding fallback)
